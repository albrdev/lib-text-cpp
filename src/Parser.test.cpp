...
namespace UnitTest
{
  ...
  TEST(Parser, NextWithPredicate)
  {
    ...
    ASSERT_TRUE(parser.GetState());
    ASSERT_EQ(parser.GetIndex(), 4u);

    parser.Next([](char c) { return std::isspace(c) != 0; });
    ASSERT_STREQ(parser.GetRemaining(), "12345");
    ASSERT_TRUE(parser.GetState());
    ASSERT_EQ(parser.GetIndex(), 5u);
